...
setwd("/home/ama/data/COI_index_seqs/unique_queries_test/coifasta/nt_blast_proks_e
uks/trinity_vs_nt//")
sk <- read.delim("subject_kingdom.map", header=FALSE, sep="\t", stringsAsFactors=F
ALSE)
colnames(sk) <- c("subj_id", "accid", "taxgroup")
##Importing the BLAST file
nt_bl <- read.delim("trinity_vs_nt.bl", header=FALSE, sep="\t", stringsAsFactors=F
ALSE)

##Adding column names
colnames(nt_bl) <- c('query', 'subject', 'title', 'sci.name','com.name', 'taxid',
'qlen',
                     'slen', 'qstart','qend', 'sstart', 'send', 'evalue','score',
'ali_length',
                     'pident', 'nident','gapopen', 'gaps', 'qcovs')

##Making an coumn with only the usable part of the subject ID
nt_bl$subj_id <- (sub("\\S+\\|(\\S+)\\|", "\\1", nt_bl$subject, perl=TRUE))

##So that it wont cause any problems during the merge process, create an array con
sisting of ony th eunique columns
sk_uniq <- sk[!duplicated(sk$subj_id),]

##Add the Superkindom information by merging the two datasets together by Subject
ID
##Note: after setting all=TRUE, I resolved issues I had with repeating mututal str
ings
nt_bl_sk <- merge(sk_uniq, nt_bl, by="subj_id", stringsAsFactors=FALSE, all=F)
tail(nt_bl_sk)


##After that I manually fix the table in a tabular format so that I can make a pie
chart later
...
nt_sk_ord <- nt_bl_sk[ order( nt_bl_sk$query, -nt_bl_sk$pident, -nt_bl_sk$score, -
nt_bl_sk$ali_length, nt_bl_sk$evalue ),]
...
nrow(nt_sk_ord_u)
#nt_dum <- nt_sk_ord[(nt_sk_ord$query == "M02443:118:000000000-AMUKU:1:1101:10448:
7230"),]
...
taxgroup_frequency(nt_sk_ord, "Overall superkingdom ratios, first ten ranking hits
 per query")
taxgroup_frequency(nt_sk_ord_u, "Overall superkingdom ratios, only top hits")
group_pie(nt_sk_ord, "Eukaryotes", "Superkingdom ratios when Eukaryotes are top hi
ts ")
group_pie(nt_sk_ord, "Prokaryotes", "Superkingdom ratios when Prokaryotes are top
hits")
...
##This function picks a stated taxonomic group eg "Eukaryotes", finds all the queri
s whose rank 1 equals
...
### We don't have species information for all of the subject ids in the nt_bl table
, so let us
### make subset of it and then use that. (This is not supposed to require more memo
ry, but
### I'm not convinced)

### Amalia, has already made this table using merge... So we can use this with tapp
ly to
### ask some questions..
...
euk.noneuk.r <- sapply( query.tg.ms.l, function(x){ (1 + x['Eukaryotes']) / (1 + ma
x(x[-2])) } )
head(euk.noneuk.r)
...
prok.nonprok.r <- sapply( query.tg.ms.l, function(x){ (1 + x['Prokaryotes']) / (1 +
 max(x[-3])) } )
hist( log2(prok.nonprok.r))

vir.nonvir.r <- sapply( query.tg.ms.l, function(x){ (1 + x['Viruses']) / (1 + max(x
[-4])) } )
hist( log2(vir.nonvir.r))

arc.nonarc.r <- sapply( query.tg.ms.l, function(x){ (1 + x['Archaea']) / (1 + max(x
[-5])) } )
hist( log2(arc.nonarc.r))


## this may not be that valid if we are only getting a single species for all the b
last alignments
## We can easily check this... by checking the accid which is the taxonomic identif
ier

query.sp.count <- tapply( 1:nrow(nt_bl_sk), nt_bl_sk$query, function(i){ length(uni
que(nt_bl_sk[i, 'accid']))})
hist(query.sp.count)
dev.off()
